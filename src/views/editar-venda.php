...
<body>
    <div class="container">
        ...
        <div class="menu p-2">
            ...
            <button id="add-item" class="btn btn-primary">Novo produto</button>
        </div>
                                    ...
                                    <option value="" selected disabled>Selecione um produto</option>
                                        <?php
                                            foreach ($produtos->findAll() as $produto) { 
                                                if ($produto->getIdProduto() == $itemVenda->getIdProduto()) { ?>
                                                    <option selected value="<?= $produto->getId() ?>"><?= $produto->getNome() ?></option> <?php
                                                } else { ?>
                                                    <option value="<?= $produto->getIdProduto() ?>"><?= $produto->getNome() ?></option> <?php
                                                }
                                            }
                                        ?>
                                </select>
                                ...
                                <select name="id-produto[]" class="form-select" id="id-produto" required>
                                    <option value="" selected disabled>Selecione um produto</option>
                                        <?php
                                            foreach ($produtos->findAll() as $produto) { 
                                                if ($produto->getIdProduto() == $itemVenda->getIdProduto()) { ?>
                                                    <option selected value="<?= $produto->getIdProduto() ?>"><?= $produto->getNome() ?></option> <?php
                                                } else { ?>
                                                    <option value="<?= $produto->getIdProduto() ?>"><?= $produto->getNome() ?></option> <?php
                                                }
                                            }
